...
以下问题在实际使用中反复出现，执行前务必检查：

### 1. 多次独立查询必须并行

涨幅榜、资金流向、估值分位等互不依赖的查询，**不要串行发起**。串行 = N × 30s，并行 ≈ 30s。用多个 Bash 调起内联 Python 脚本，一次并发出去。

### 2. Windows 下不要用 CLI，用内联 Python

`scripts/cli.py` 在 Windows 终端输出中文必然乱码（GBK/UTF-8 冲突），且依赖 `python3` 命令，Windows 上只有 `python`。**Windows 环境一律用内联 Python 脚本**，并加：
...
macOS / Linux 则两者皆可，CLI 正常。

### 3. 返回字段名不固定，先查 keys 再取值

不同查询返回的字段名不同：
- 涨幅查询 → `涨跌幅[20260618]` 或 `最新涨跌幅:前复权`
- 资金查询 → `主力净买入额[20260618]`

**每次查询先打印 `list(datas[0].keys())`，确认字段名后再取数**，不要硬编码字段名。尤其注意方括号和冒号这些非标准字符。

### 4. 多查询组合时优先做交叉分析

拿到涨幅 + 资金数据后，交叉对比能发现"量价背离"信号（如涨幅高但资金流出、或资金流入但价格不涨），这些是用户真正关心的洞察。单纯罗列两张表不如点出交叉信号。
